...
🔋 (PHP / Grav) Production-ready Docker images with automatic Grav installer.

- Docker Hub: https://hub.docker.com/r/shinsenter/grav
- GitHub Packages: https://code.shin.company/php/pkgs/container/grav
...
# For example:
# ADD --chown=$APP_USER:$APP_GROUP ./grav/ /var/www/html/
```
...
But you can pull the image from `shinsenter/grav:latest`,
and tag it with a name that indicates its stability,
such as `your-repo/grav:stable` using the below commands:

```shell
docker pull shinsenter/grav:latest
docker tag  shinsenter/grav:latest your-repo/grav:stable
docker push your-repo/grav:stable
```

Then use the image `your-repo/grav:stable` as a base image to build containers for production.
