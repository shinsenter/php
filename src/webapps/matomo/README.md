...
🔋 (PHP / Matomo) Production-ready Docker images with automatic Matomo Open Source installer.

- Docker Hub: https://hub.docker.com/r/shinsenter/matomo
- GitHub Packages: https://code.shin.company/php/pkgs/container/matomo
...
services:
  web:
    image: shinsenter/matomo:latest
    volumes:
      - ./matomo:/var/www/html
...
But you can pull the image from `shinsenter/matomo:latest`,
and tag it with a name that indicates its stability,
such as `your-repo/matomo:stable` using the below commands:

```shell
docker pull shinsenter/matomo:latest
docker tag  shinsenter/matomo:latest your-repo/matomo:stable
docker push your-repo/matomo:stable
```

Then use the image `your-repo/matomo:stable` as a base image to build containers for production.
