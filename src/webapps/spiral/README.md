...
🔋 (PHP / Spiral) Production-ready Docker images with automatic Spiral installer.

- Docker Hub: https://hub.docker.com/r/shinsenter/spiral
- GitHub Packages: https://code.shin.company/php/pkgs/container/spiral
...
docker run -p 80:80 -p 443:443 -p 443:443/udp \
    -v ./spiral:/var/www/html \
    shinsenter/spiral:latest
```
...
# For example:
# ADD --chown=$APP_USER:$APP_GROUP ./spiral/ /var/www/html/
```
...
services:
  web:
    image: shinsenter/spiral:latest
    volumes:
      - ./spiral:/var/www/html
...
But you can pull the image from `shinsenter/spiral:latest`,
and tag it with a name that indicates its stability,
such as `your-repo/spiral:stable` using the below commands:

```shell
docker pull shinsenter/spiral:latest
docker tag  shinsenter/spiral:latest your-repo/spiral:stable
docker push your-repo/spiral:stable
```

Then use the image `your-repo/spiral:stable` as a base image to build containers for production.
