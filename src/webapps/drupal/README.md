...
But you can pull the image from `shinsenter/drupal:latest`,
and tag it with a name that indicates its stability,
such as `shinsenter/drupal:stable` using the below commands:

```shell
docker pull shinsenter/drupal:latest
docker tag  shinsenter/drupal:latest your-repo/drupal:stable
docker push your-repo/drupal:stable
```

Then use the image `your-repo/drupal:stable` as a base image to build containers for production.
