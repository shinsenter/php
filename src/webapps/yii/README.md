...
- GitHub Packages: https://github.com/shinsenter/php/pkgs/container/yii
...
But you can pull the image from `shinsenter/yii:latest`,
and tag it with a name that indicates its stability,
such as `shinsenter/yii:stable` using the below commands:

```shell
docker pull shinsenter/yii:latest
docker tag  shinsenter/yii:latest your-repo/yii:stable
docker push your-repo/yii:stable
```

Then use the image `your-repo/yii:stable` as a base image to build containers for production.
