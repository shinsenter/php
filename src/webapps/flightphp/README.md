...
🔋 (PHP / Flight) Production-ready Docker images with automatic Flight framework installer.

- Docker Hub: https://hub.docker.com/r/shinsenter/flightphp
- GitHub Packages: https://code.shin.company/php/pkgs/container/flightphp
...
docker run -p 80:80 -p 443:443 -p 443:443/udp \
    -v ./flightphp:/var/www/html \
    shinsenter/flightphp:latest
```
...
```Dockerfile
FROM shinsenter/flightphp:latest
...
# For example:
# ADD --chown=$APP_USER:$APP_GROUP ./flightphp/ /var/www/html/
```
...
services:
  web:
    image: shinsenter/flightphp:latest
    volumes:
...
But you can pull the image from `shinsenter/flightphp:latest`,
and tag it with a name that indicates its stability,
such as `your-repo/flightphp:stable` using the below commands:

```shell
docker pull shinsenter/flightphp:latest
docker tag  shinsenter/flightphp:latest your-repo/flightphp:stable
docker push your-repo/flightphp:stable
```

Then use the image `your-repo/flightphp:stable` as a base image to build containers for production.
