...
🔋 (PHP / Kirby) Production-ready Docker images with automatic Kirby installer.

- Docker Hub: https://hub.docker.com/r/shinsenter/kirby
- GitHub Packages: https://code.shin.company/php/pkgs/container/kirby
...
docker run -p 80:80 -p 443:443 -p 443:443/udp \
    -v ./kirby:/var/www/html \
    shinsenter/kirby:latest
```
...
# For example:
# ADD --chown=$APP_USER:$APP_GROUP ./kirby/ /var/www/html/
```
...
But you can pull the image from `shinsenter/kirby:latest`,
and tag it with a name that indicates its stability,
such as `your-repo/kirby:stable` using the below commands:

```shell
docker pull shinsenter/kirby:latest
docker tag  shinsenter/kirby:latest your-repo/kirby:stable
docker push your-repo/kirby:stable
```

Then use the image `your-repo/kirby:stable` as a base image to build containers for production.
