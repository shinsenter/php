...
ARG BUILD_FROM_IMAGE=${BUILD_FROM_IMAGE:-shinsenter/wordpress}
...
RUN env-default ENABLE_CRONTAB   '$(is-true "$DISABLE_WP_CRON" && echo 0 || echo 1)'
RUN env-default CRONTAB_SETTINGS '*/5 * * * * curl http://localhost/wp/wp-cron.php 2>&1'
