...
🔋 (PHP / EspoCRM) Production-ready Docker images with automatic EspoCRM installer.

- Docker Hub: https://hub.docker.com/r/shinsenter/espocrm
- GitHub Packages: https://code.shin.company/php/pkgs/container/espocrm
...
```Dockerfile
FROM shinsenter/espocrm:latest
...
But you can pull the image from `shinsenter/espocrm:latest`,
and tag it with a name that indicates its stability,
such as `your-repo/espocrm:stable` using the below commands:

```shell
docker pull shinsenter/espocrm:latest
docker tag  shinsenter/espocrm:latest your-repo/espocrm:stable
docker push your-repo/espocrm:stable
```

Then use the image `your-repo/espocrm:stable` as a base image to build containers for production.
