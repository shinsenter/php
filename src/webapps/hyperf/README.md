...
🔋 (PHP / Hyperf) Production-ready container with automatic Hyperf installer.

- Docker Hub: https://hub.docker.com/r/shinsenter/hyperf
- GitHub Packages: https://github.com/shinsenter/php/pkgs/container/hyperf
...
But you can pull the image from `shinsenter/hyperf:latest`,
and tag it with a name that indicates its stability,
such as `shinsenter/hyperf:stable` using the below commands:

```shell
docker pull shinsenter/hyperf:latest
docker tag  shinsenter/hyperf:latest your-repo/hyperf:stable
docker push your-repo/hyperf:stable
```

Then use the image `your-repo/hyperf:stable` as a base image to build containers for production.
